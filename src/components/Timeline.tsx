...
function CompactLivePreview({ url, title }: { url: string; title: string }) {
  return (
    <div className="group mt-7 overflow-hidden rounded-2xl border border-navy/10 bg-white shadow-[0_18px_45px_rgba(10,25,47,.08)] transition-all duration-500 hover:-translate-y-1 hover:border-accent/25">
      <div className="flex h-9 items-center gap-1.5 border-b border-neutral-200 bg-[#f2f3f6] px-3">
        ...
        <a href={url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-[10px] font-semibold text-navy/55 transition-colors hover:text-accent" aria-label={`在新页面打开 ${title}`}>
          打开 <ExternalLink className="h-3.5 w-3.5" />
        </a>
      </div>
      <div className="relative h-56 bg-white md:h-64">
        <iframe src={url} title={`${title} 互动网页`} className="h-full w-full border-0" loading="lazy" />
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 items-center">
      ...
      >
        {item.slideImages ? (
          ...
          <div className="group relative flex h-full w-full flex-col bg-[#F5F5F7]">
            ...
            <div className="relative w-full flex-1 overflow-hidden bg-white">
              <iframe
                ...
                title={`${item.title} 互动网页`}
                loading="lazy"
              />
            </div>
            <a href={item.liveDemoUrl} target="_blank" rel="noreferrer" className="absolute bottom-4 right-4 inline-flex items-center gap-2 rounded-full border border-white/15 bg-[#071326]/85 px-4 py-2 text-xs font-semibold text-white shadow-xl backdrop-blur-xl transition-transform duration-300 hover:-translate-y-1" aria-label={`在新页面打开 ${item.title}`}>
